                ...
                if current_time - self.last_process_time >= self.process_interval:
                    ...
                    if plate_number is not None and self.is_valid_plate_number(plate_number):
                        ...
                        if self.isEntering:
                            ...
                            cursor.execute(f"SELECT * FROM car_entry_exit_log WHERE carplate = '{plate_number}' AND leave_at IS NULL ORDER BY enter_at DESC LIMIT 1")
                        ...
                        # if the car is exiting
                            ...
                            if record is None:
                                ...
                                cursor = database.cursor(dictionary=True)
                                cursor.execute(f"UPDATE car_entry_exit_log SET leave_at = NOW() WHERE carplate = '{plate_number}' ORDER BY enter_at DESC LIMIT 1")
                                database.commit()
